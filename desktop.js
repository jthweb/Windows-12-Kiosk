...
function addMenu() {
    ...
    for (var i = 0; i < childDivs.length; i++) {
        if (i <= 4) {//win12<<TRANSLATE:内置的>>5<<TRANSLATE:个图标不添加>>
            continue;
        }
        var div = childDivs[i];
...
var run_cmd = '';
let nomax = { 'calc': 0 /* Actually, the calculator can be maximized...*/, 'notepad-fonts': 0, 'camera-notice': 0, 'winver': 0, 'run': 0, 'wsa': 0 };
...
window.onkeydown = function (event) {
    if (event.keyCode == 116/*F5<<TRANSLATE:被按下>>(Refresh)*/) {
...
function showcm(e, cl, arg) {
    if ($('#cm').hasClass('show-begin')) {
        setTimeout(() => {
            ...
            $('#cm>.foc').focus();
            // <<TRANSLATE:<<TRANSLATE:这>>个>>.foc<<TRANSLATE:是用来模拟焦点的>>，<<TRANSLATE:<<TRANSLATE:这>>句是将焦点放<<TRANSLATE:在>>右键菜单上>>，<<TRANSLATE:注释掉后果不堪设想>> >u-)o
            // <<TRANSLATE:噢>> <<TRANSLATE:可是如果<<TRANSLATE:设置>>焦点的话<<TRANSLATE:在>>移动设备上会显示虚拟键盘啊>> QAQ (By: User782Tec)
            // (By: tjy-gitnub)
            setTimeout(() => {
                $('#cm').addClass('show');
            }, 0);
...
let nts = {
    ...
    'ZeroDivision': {//<<TRANSLATE:计算器报错窗口>>
        // <<TRANSLATE:甚至还报错<<TRANSLATE:我>>真的哭死>>，<<TRANSLATE:直接输入框显示给>>error<<TRANSLATE:啥的不就完了>>。。
...
var shutdown_task = []; //<<TRANSLATE:Shut Down任务>>，储存<<TRANSLATE:在>><<TRANSLATE:<<TRANSLATE:这>>个>>数组里
// <<TRANSLATE:为什么要数组>>？
// <<TRANSLATE:Run的指令>>
function runcmd(cmd) {
    ...
    else if (cmd.includes("shutdown")) {//<<TRANSLATE:Shut Down指令>>
        run_cmd = cmd
        var cmds = cmd.split(' ');
        if (cmds.includes("shutdown") || cmds.includes("shutdown.exe")) { //<<TRANSLATE:帮助>>
            if (cmds.length == 1) {
                openapp('terminal');
                $('#win-terminal').html(`
<pre>
...
-t time: Specifies to operate after time seconds<<TRANSLATE:作>>

I won’t introduce much else.
Please press any key to continue.&nbsp;.&nbsp;.<input type="text" onkeydown="hidewin('terminal')"></input></pre>`); //Q：<<TRANSLATE:为什么文字<<TRANSLATE:这>>么多呢>>？A：shutdown的<<TRANSLATE:帮助>>本来就多，<<TRANSLATE:为了能显示空格>>，<<TRANSLATE:就把空格用>>&nbsp;<<TRANSLATE:代替了>>
                // <<TRANSLATE:所以你是没事干吗>>？。。Prompt：github<<TRANSLATE:并不是以行数来计算贡献的哦>>   from @tjy-gitnub
                $('#win-terminal>pre>input').focus()
            }
            else if (cmds.includes("-s") || cmds.includes("/s")) {//Shut Down
                if ((cmds.indexOf("-t") != -1 && cmd.length/*<<TRANSLATE:判断是否>>-t<<TRANSLATE:后有其他参数>>*/ >= cmds.indexOf("-t") + 2/*<<TRANSLATE:先加一>>，<<TRANSLATE:获取当下标是从>>1<<TRANSLATE:<<TRANSLATE:开始的>>时候的下标索引>>；<<TRANSLATE:再加一>>，<<TRANSLATE:获取下一项>>。<<TRANSLATE:配合数组>>.length<<TRANSLATE:使用>>*/) || (cmds.indexOf("/t") != -1 && cmd.length/*<<TRANSLATE:判断是否>>-t<<TRANSLATE:后有其他参数>>*/ >= cmds.indexOf("/t") + 2)) {
                    str = "";
                    if (cmds.includes("-t")) { str = "-t"; }
                    if (cmds.includes("/t")) { str = "/t"; }
                    if (!isNaN(cmds[cmds.indexOf(str) + 1]/*<<TRANSLATE:<<TRANSLATE:这>>里只加一是因为下标是从>>0<<TRANSLATE:开始的>>*/)) {
                        num = parseInt(cmds[cmds.indexOf(str) + 1])
                        nts['shutdown'] = {
                            cnt: `
                            <p class="tit">You will be logged out soon</p>
                            <p>Windows will shut down after ` + calcTimeString(num) + `. </p>`, // <<TRANSLATE:如果必须原生样式的>>，<<TRANSLATE:建议改为>> num<60 ? 1 : Math.floor(num / 60) + ` <<TRANSLATE:Minute后关闭>>。</p>`,
                            btn: [
                                { type: 'main', text: 'Close', js: 'closenotice();' },
                            ]
                        };
                ...
                if ((cmds.indexOf("-t") != -1 && cmd.length >= cmds.indexOf("-t") + 2) || (cmds.indexOf("/t") != -1 && cmd.length >= cmds.indexOf("/t") + 2)) {/*<<TRANSLATE:详见上面的注释>>*/
                    str = "";
                    if (cmds.includes("-t")) { str = "-t"; }
                    if (cmds.includes("/t")) { str = "/t"; }
            ...
            else if (cmds.includes("-a") || cmds.includes("/a")) {//取消电源操<<TRANSLATE:作>>
                if (shutdown_task.length > 0) {
                    ...
                    nts['shutdown'] = {
                        ...
                        <p>The planned Close has been cancelled.</p>`,
                        btn: [
                            { type: 'main', text: 'Close', js: 'closenotice();' },
                        ]
                    };
...
let apps = {
                    ...
                    if (sha != json[0].sha) {
                        ...
                        if (msg.match(/v[0-9]*\.[0-9]*\.[0-9]*/)) {
                            ...
                            window.setTimeout(() => {
                                ...
                                $('#win-setting>.page>.cnt.update>.lo>.update-main .detail')[0].innerText = `Last check time: ${da.getFullYear()}Year${da.getMonth() + 1}Month${da.getDate()}Day，${da.getHours()}: ${da.getMinutes()}`;
                                $('#win-setting>.page>.cnt.update>.lo>.update-main>div:last-child').removeClass('disabled');
    ...
    run: {
        init: () => {
            $('#win-run>.open>input').val(run_cmd);   //<<TRANSLATE:在>>windows<<TRANSLATE:中>>，<<TRANSLATE:Run输入的内容会被保留>>
            window.setTimeout(() => {
    ...
    taskmgr: {
        ...
        generateProcesses: () => {
            ...
            for (const elt of apps.taskmgr.tasks) {
                ...
                if (document.getElementById('tsk-search').value != '' && document.getElementById('tsk-search').style.display == '' && (!elt.name.toLowerCase().includes(document.getElementById('tsk-search').value.toLowerCase()/* <<TRANSLATE:Search时转换成小写>> */))) {
                    continue //Search
                }
        ...
        loadProcesses: (processList = apps.taskmgr.processList) => {
            ...
            for (const elt of processList) {
                ...
                newElt.innerHTML = `<td><div class="text"><div class="icon" style="background-image: url('${elt.icon ? elt.icon : ''}');"></div>${elt.name}</div></td><td style="text-align: right;background-color: ${page.classList.contains('dark') ? '#193662' : '#7ec6ec'}${elt.cpu >= (max / 1.3) ? 'ff' : 'aa'};">${elt.cpu.toFixed(1)}%</td><td style="text-align: right;background-color: ${page.classList.contains('dark') ? '#193662' : '#7ec6ec'}${elt.memory >= (max / 1.3) ? 'ff' : 'aa'};">${elt.memory.toFixed(1)}%</td><td style="text-align: right;background-color: ${page.classList.contains('dark') ? '#193662' : '#7ec6ec'}${elt.disk >= (max / 1.3) ? 'ff' : 'aa'};">${elt.disk.toFixed(1)}%</td><td>${['<<TRANSLATE:非常<<TRANSLATE:低>>>>', '<<TRANSLATE:非常<<TRANSLATE:低>>>>', '<<TRANSLATE:非常<<TRANSLATE:低>>>>', '<<TRANSLATE:低>>', '<<TRANSLATE:中>>'][Math.floor(Math.random() * 5)]}</td>`;
        ...
        performanceLoad: () => {
            ...
            $('#win-taskmgr>.main>.cnt.performance>.content>.performance-graph>.graph-memory>.information>.left>div:nth-child(2)>.value')[0].innerText = `${(64 - apps.taskmgr.memory).toFixed(1)} GB`;
            $('#win-taskmgr>.main>.cnt.performance>.content>.performance-graph>.graph-memory>.information>.left>div:nth-child(3)>.value')[0].innerText = `${(apps.taskmgr.memory - 2).toFixed(1)} / 64 GB`;
            ...
            $('#win-taskmgr>.main>.cnt.performance>.content>.performance-graph>.graph-disk>.information>.left>div:nth-child(2)>.value')[0].innerText = `${apps.taskmgr.disk != 0 ? Math.random().toFixed(2) : 0} <<TRANSLATE:毫Second>>`;
            $('#win-taskmgr>.main>.cnt.performance>.content>.performance-graph>.graph-disk>.information>.left>div:nth-child(3)>.value')[0].innerText = `${apps.taskmgr.diskSpeed.read} ${apps.taskmgr.disk != 0 ? 'MB/Second' : 'KB/Second'}`;
            $('#win-taskmgr>.main>.cnt.performance>.content>.performance-graph>.graph-disk>.information>.left>div:nth-child(4)>.value')[0].innerText = `${apps.taskmgr.diskSpeed.write} ${apps.taskmgr.disk != 0 ? 'MB/Second' : 'KB/Second'}`;
            ...
            $('#win-taskmgr>.main>.cnt.performance>.content>.select-menu>.graph-wifi>.right>.data>.value2')[0].innerText = `<<TRANSLATE:发送>>: ${apps.taskmgr.wifi[1]} <<TRANSLATE:接收>>: ${apps.taskmgr.wifi[0]} Mbps`;
        },
        graphLoad: () => {
    ...
    explorer: {
        init: () => {
            ...
            apps.explorer.is_use = 0;//<<TRANSLATE:千万不要删除它>>，<<TRANSLATE:它依托>>bugRun
            apps.explorer.is_use2 = 0;//<<TRANSLATE:千万不要删除它>>，<<TRANSLATE:它依托>>bugRun
    ...
    box-shadow: 0 1px 2px var(--s3d);
            ...
            </div><p class="info">32.6 GB <<TRANSLATE:可用>>, <<TRANSLATE:共>> 143 GB</p></div></a><a class="a item act" ondblclick="apps.explorer.goto('D:')" ontouchend="apps.explorer.goto('D:')"
            ...
            </div><p class="info">185.3 GB <<TRANSLATE:可用>>, <<TRANSLATE:共>> 216 GB</p></div></a></div>`;
        ...
        copy_or_cut: (path, operate) => { //operate<<TRANSLATE:只能为>>copy<<TRANSLATE:或>>cut
        ...
        paste: (path) => {
            ...
            if (apps.explorer.traverseDirectory(tmp, clipboard[1][0]))
                // {
                //     clipboard[1][0] += " - <<TRANSLATE:副本>>";
                // }
                // if (apps.explorer.traverseDirectory(tmp,clipboard[1]['name']))
                // {
                //     clipboard[1][0] += " - <<TRANSLATE:副本>>";
                // }
                // <<TRANSLATE:<<TRANSLATE:这>>段注释了的代码可以调试一下>>，<<TRANSLATE:会有神奇的>>bug。
        ...
        del_select: () => {
            ...
            for (var i = 0; i < elements.length; i++) {
                ...
                //<<TRANSLATE:<<TRANSLATE:这>>边可以适配更多的文件类型>>
        ...
        add: (path, name_, type = "file", command = "", icon = "") => { //type<<TRANSLATE:为文件类型>>，<<TRANSLATE:只有文件夹>>files<<TRANSLATE:<<TRANSLATE:和>>文件>>file
            var pathl = path.split('/');
            var icon_ = "";
            let tmp = apps.explorer.path;
            pathl.forEach(name => {
        ...
        get_file_id: (name) => {  //只能找到已经Open了的文件夹的元素id
        ...
        // <<TRANSLATE:禁止奇奇怪怪的缩进>>！<<TRANSLATE:尽量压行>>，<<TRANSLATE:不要毫无意义地全部格式化<<TRANSLATE:和>>展开>>！ // f*ck!<<TRANSLATE:这>>™<<TRANSLATE:的格式化工具>>，<<TRANSLATE:我>>*********
        path: {
            folder: {
                'C:': {
                    folder: {
                            ...
                            folder: {
                                'Boot': { folder: {}, file: [] }, 'System': { folder: {}, file: [] }, 'SysWOW64': { folder: {}, file: [] }, 'System32': {
                                    folder: {}, file: [
                                        ...
                                        { name: 'notepad.exe', ico: 'icon/notepad.svg', command: "openapp('notepad')" },//system32<<TRANSLATE:也有一个>>notepad
                            ...
                            folder: {
                                ...
                                'Public': {
                                    folder: {
                                        ...
                                        'Public m': { folder: { '<<TRANSLATE:录音机>>': { folder: {}, file: [] } } }
                                    }}}}},
                    file: []
                },
                'D:': {
                    ...
                    file: [
                        { name: '<<TRANSLATE:瓶盖结构说明>>.docx', ico: 'icon/files/word.png', command: '' },
                        { name: '<<TRANSLATE:可口可乐瓶盖历史>>.pptx', ico: 'icon/files/ppt.png', command: '' },
                    ]}}},
...
(c) Microsoft Corporation. All rights reserved.
    ...
    search: {
        rand: [{ name: 'Nongfu Spring bottle cap introduction.txt', bi: 'text', ty: 'text document' },
        { name: 'Bottle cap structure diagram.png', bi: 'image', ty: 'PNG file' },
        { name: 'Bottle cap structure description.docx', bi: 'richtext', ty: 'Microsoft Word document' },
        { name: 'Coca-Cola bottle cap.jpg', bi: 'image', ty: 'JPG file' },
        { name: 'History of Coca-Cola bottle caps.pptx', bi: 'slides', ty: 'Microsoft Powerpoint presentation' },
        { name: 'Statistical analysis of bottle cap quality.xlsx', bi: 'ruled', ty: 'Microsoft Excel worksheet' },
        { name: 'Nongfu Spring bottle cap.svg', bi: 'image', ty: 'SVG file' },
        { name: 'Bottle cap introduction.doc', bi: 'richtext', ty: 'Microsoft Word document' }],
        search: le => {
            if (le > 0) {
    ...
    edge: {
        ...
        in_div(id,event) {
            ...
            if (x < divx1 || x > divx2 || y < divy1 || y > divy2) {
                //<<TRANSLATE:如果离开>>，<<TRANSLATE:则执行>>。。 
                return false;
            }
            else {
                //<<TRANSLATE:如检播到>>，<<TRANSLATE:则执行>>。。 
                return true;
            }
        },
        settab: (t, i) => {
        ...
        goto: (u, clear = true) => {
            if (wifiStatus == false) {
                ...
                // Check if URL containshttp<<TRANSLATE:头>>
...
let copilot = {
    ...
    init: () => {
        ...
        $('#copilot>.chat').append(`<div class="line system"><p class="text">正<<TRANSLATE:在>>初始化...</p></div>`);
        $('#copilot>.chat').scrollTop($('#copilot>.chat')[0].scrollHeight);
    },
    send: (t, showusr = true) => {
        $('#copilot>.inputbox').addClass('disable');
        if (t.length == 0) {
            $('#copilot>.chat').append(`<div class="line system"><p class="text"><<TRANSLATE:系统表示请发一些有意义的东西>></p></div>`);
            $('#copilot>.chat').scrollTop($('#copilot>.chat')[0].scrollHeight);
...
// Day期、Time
let da = new Date();
let date = `Week${['Sunday ', ' Monday ', ' Tuesday ', ' Wednesday ', ' Thursday ', ' Friday ', ' Saturday '][da.getDay()]}, ${da.getFullYear()}Year${(da.getMonth() + 1).toString().padStart(2, '0')}Month${da.getDate().toString().padStart(2, '0')}Day`
...
apps.setting.theme_get();//<<TRANSLATE:提前加载Theme>>
loadtime();
setTimeout('loadtime();setInterval(loadtime, 1000);', 1000 - da.getMilliseconds());//修复Time不精准的问题。<<TRANSLATE:以前的误差>>：0-999<<TRANSLATE:毫Second>>；现<<TRANSLATE:在>>：<<TRANSLATE:几乎没有>>
let d = new Date();
...
function orderwindow() {
    for (let i = 0; i < wo.length; i++) {
        ...
        if (topmost.includes(wo[wo.length - i - 1])) {
            win.css('z-index', 10 + i + 50/*<<TRANSLATE:这>>里的50<<TRANSLATE:可以改>>，<<TRANSLATE:不要太大>>，<<TRANSLATE:不然会覆盖任务栏>>；<<TRANSLATE:不要太小>>，<<TRANSLATE:不然就<<TRANSLATE:和>>普通窗口没有什么区别了>>。随着Version的更新，<<TRANSLATE:肯定会有更多窗口>>，<<TRANSLATE:以后就可以把数字改打一点点>>*/);
        } else {
            win.css('z-index', 10 + i);
        }
    }
}
// <<TRANSLATE:以下函数基于>>bugRun，<<TRANSLATE:切勿改动>>！
...
// Control Panel Power Monitoring
try {
...
if (isDarkTheme.matches) { //<<TRANSLATE:是深色>>
    $('.dock.theme').toggleClass('dk');
    $(':root').toggleClass('dark');
    $('.window.whiteboard>.titbar>p').text('Blackboard');
    localStorage.setItem('theme', 'dark');
    isDrak = true;
} else { // 不<<TRANSLATE:是深色>>
    $('.window.whiteboard>.titbar>p').text('Whiteboard');
...
function setIcon() {
    ...
    if (Array.isArray(JSON.parse(localStorage.getItem('sys_setting')))) {
        ...
        if (/^(1|0)+$/.test(sys_setting_back.join(''))/* <<TRANSLATE:只含有>>0<<TRANSLATE:和>>1 */) {
            sys_setting = sys_setting_back;
            for (var i = 0; i < sys_setting.length; i++) {
                document.getElementById('sys_setting_' + (i + 1)).setAttribute("class", 'a checkbox' + (sys_setting[i] ? ' checked' : '')); //<<TRANSLATE:设置>>classProperties
                if (i == 5) {
...
document.getElementsByTagName('body')[0].onload = function nupd() {
    ...
    setIcon();//<<TRANSLATE:加载桌面图标>>

    // 所以<<TRANSLATE:<<TRANSLATE:这>>个>>东西为啥要<<TRANSLATE:在>>开机的时候加载？
    // 不应该<<TRANSLATE:在>>python.init<<TRANSLATE:里面吗>>？
    //     (async function () {
    //         apps.python.pyodide = await loadPyodide();
    //         apps.python.pyodide.runPython(`
    // import sys
...
if (!location.href.match(/((\d|[1-9]\d|1\d\d|2[0-4]\d|25[0-5])\.){3}(\d|[1-9]\d|1\d\d|2[0-4]\d|25[0-5])(?::(?:[0-9]|[1-9][0-9]{1,3}|[1-5][0-9]{4}|6[0-4][0-9]{3}|65[0-4][0-9]{2}|655[0-2][0-9]|6553[0-5]))/) && !(new URL(location.href)).searchParams.get('develop')) {
    ...
    navigator.serviceWorker.register('sw.js', { updateViaCache: 'none', scope: './' }).then(reg => {
        ...
        reg.addEventListener('updatefound', () => {
            // 正<<TRANSLATE:在>>安装的新的 SW
            const newWorker = reg.installing;
            console.log('dsk-<<TRANSLATE:发现更新>>');
            // newWorker.state;
            // // "installing" - Install event triggered，<<TRANSLATE:但还没完成>>
            // // "installed"  - Installation Complete
            // // "activating" - Activate event triggered，<<TRANSLATE:但还没完成>>
            // // "activated"  - Activation Successful
            // // "redundant"  - <<TRANSLATE:废弃>>，<<TRANSLATE:可能是因为安装失败>>，<<TRANSLATE:或>>者是被一个新Version覆盖
        });
    });
    ...
    function setData(k, v) {
        console.log('setData <<TRANSLATE:被禁用>>');
    }
}
...
/**
 * 将Second数换算为可读的Time格式
 * @param {number} second Second数
 * @returns 将Second数格式化为  1 <<TRANSLATE:天>> 8 Hour 43 Minute 26 Second类似的格式
 */
